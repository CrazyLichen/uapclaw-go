...
| 2.7 | ✅ | OpenAI 兼容客户端 | HTTP 调用（invoke + stream），SSE 解析；⤵️ **2.6 回填点已实现**：`init()` 中已调用 `Register("OpenAI", "llm", ...)` + `Register("OpenRouter", "llm", ...)`；⤵️ **2.13 回填点**：`request_builder.go` 中 `SanitizeHeaders`/`MergeHeaders` 待 2.13 统一收敛；⤵️ **2.14 回填点**：回调框架 `trigger(LLMCallEvents.*)` 当前用日志替代，待 2.14 实现后回填；⤵️ **2.16 回填点**：Stream 中 `_astream_with_parser` 逻辑待 2.16 实现后回填；⤵️ **2.14/2.15 回填点**：需 blank import `openai` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/model_clients/openai_model_client.py` |
...
| 2.9 | ✅ | DeepSeek 客户端 | DeepSeek API 适配；✅ **2.6 回填点已实现**：`init()` 中已调用 `Register("DeepSeek", "llm", ...)`；⤵️ **2.7 回填点**：`SanitizeHeaders`/`MergeHeaders` 待 2.13 统一收敛；⤵️ **2.14 回填点**：回调框架 `trigger(LLMCallEvents.*)` 当前用日志替代，待 2.14 实现后回填；⤵️ **2.16 回填点**：Stream 中 `_astream_with_parser` 逻辑待 2.16 实现后回填；⤵️ **2.14/2.15 回填点**：需 blank import `deepseek` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/model_clients/deepseek_model_client.py` |
| 2.10 | ✅ | SiliconFlow 客户端 | SiliconFlow 适配；✅ **2.6 回填点已实现**：`init()` 中已调用 `Register("SiliconFlow", "llm", ...)`；⤵️ **2.14/2.15 回填点**：需 blank import `siliconflow` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/model_clients/siliconflow_model_client.py` |
| 2.11 | ✅ | InferenceAffinity 客户端 | InferenceAffinity (vLLM) 适配，嵌入 OpenAI 客户端复用 HTTP/SSE/解析，覆写 Invoke/Stream（sanitize tool_calls + cache_sharing/cache_salt 参数注入），实现 Release()（KV Cache 释放）；✅ **2.6 回填点已实现**：`init()` 中已调用 `Register("InferenceAffinity", "llm", ...)`；BaseModelClient 接口新增 Release 方法，其他客户端返回不支持错误；⤵️ **2.13 回填点**：`sanitizeToolCalls` 待 2.13 统一收敛到 headers_helper；⤵️ **2.14/2.15 回填点**：需 blank import `inference_affinity` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/model_clients/inference_affinity_model_client.py` |
| 2.12 | ✅ | IntelliRouter 客户端 | IntelliRouter 智能路由适配，自行实现 ReliableRouter（四种策略：simple-shuffle/round-robin/lowest-latency/adaptive），嵌入 OpenAI 客户端复用 HTTP/SSE/响应解析，覆写 Invoke（带重试的路由选择→替换 api_key/api_base→委托 OpenAI）/Stream（选 deployment→委托 OpenAI），BaseClientEmbed 新增 WithSkipValidate 选项，路由缓存共享（MD5 key + double-check locking），Deployment 运行时状态管理（健康/延迟/统计）；✅ **2.6 回填点已实现**：`init()` 中已调用 `Register("intelli_router", "llm", ...)`；⤵️ **2.14 回填点**：回调框架 `trigger(LLMCallEvents.*)` 当前用日志替代，待 2.14 实现后回填；⤵️ **2.14/2.15 回填点**：需 blank import `intellirouter` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/model_clients/intelli_router_model_client.py` |
| 2.13 | ☐ | Headers Helper | 模型请求头构建辅助；⤴️ **回填 2.7**：将 `openai/request_builder.go` 中的 `SanitizeHeaders`/`MergeHeaders` 统一收敛到此处 | `openjiuwen/core/foundation/llm/headers_helper.py` |
| 2.14 | ☐ | Model 门面 | 统一入口，集成回调框架；⤴️ **回填 2.7**：将 `openai/client.go` 中的日志记录替换为 `trigger(LLMCallEvents.*)` 回调触发 | `openjiuwen/core/foundation/llm/model.py` |
| 2.15 | ☐ | init_model 工厂 | 便捷创建 Model 实例；⤴️ **回填 2.7**：需 blank import `openai` 包触发 `init()` 注册 | `openjiuwen/core/foundation/llm/__init__.py` (init_model) |
| 2.16 | ☐ | 输出解析器 | `JsonOutputParser`，`MarkdownOutputParser`；⤵️ **2.6 回填点**：扩展 `model_clients.BaseOutputParser` 接口，添加完整方法；⤴️ **回填 2.7**：在 `openai/client.go` Stream 中实现 `_astream_with_parser` 流式解析逻辑 | `openjiuwen/core/foundation/llm/output_parsers/output_parser.py` |
...
| 5.6 | ☐ | SessionController | 会话控制器 | `openjiuwen/core/session/session_controller/` |
| 5.7 | ☐ | Interaction | 交互管理 | `openjiuwen/core/session/interaction/` |
| 5.8 | ☐ | Checkpointer 接口 + 工厂 | `CheckpointerFactory`，InMemory 实现 | `openjiuwen/core/session/checkpointer/base.py` · `factory.py` |
...
| 5.22 | ☐ | DialogueCompressor | 对话压缩器 | `openjiuwen/core/context_engine/processor/` (DialogueCompressor) |
| 5.23 | ☐ | MicroCompactProcessor | 微压缩处理器 | `openjiuwen/core/context_engine/processor/` |
| 5.24 | ☐ | FullCompactProcessor | 全量压缩处理器 | `openjiuwen/core/context_engine/processor/` |
| 5.25 | ☐ | CurrentRoundCompressor | 当轮压缩器 | `openjiuwen/core/context_engine/processor/` |
