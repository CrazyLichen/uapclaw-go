...
| 6.8 | ✅ | @rail 装饰器等价 | ✅ RailExecutor 结构体(before/gate/exception/retry/after 完整流程)；✅ fireAfter 辅助方法(CancelledError 保护+异常掩盖处理)；✅ isCancelled 辅助函数；✅ ModelCallRail/ToolCallRail 预定义变量；✅ ConsumeRetryRequest/HasForceFinishRequest 从 panic 桩改为安全桩(⤵️ 6.10 回填)；✅ 测试全部通过 | `openjiuwen/core/single_agent/rail/base.py` (@rail decorator) |
