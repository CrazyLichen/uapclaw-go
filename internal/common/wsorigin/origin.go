...
//
// Python: jiuwenswarm/common/security/ws_origin.py 全部函数
//
...
// Python: _FORBIDDEN_BODY = b"Forbidden: Origin not allowed\n"
...
//
// Python: is_origin_check_enabled()
...
//
// Python: get_allowed_origin_hosts()
// 每次调用重新读取环境变量，支持热生效。
...
//
// Python: is_allowed_browser_origin(origin)
...
func (c *OriginChecker) IsAllowed(origin string) bool {
	...
	// Python: if origin is None: return "none" in allowed_hosts
	if origin == "" {
	...
	// 取 hostname 小写匹配白名单
	// Python: (parsed.hostname or "").lower() in allowed_hosts
...
//
// Python: forbidden_origin_response()
