...
//   - 分组件文件输出（gateway.log/channel.log/agent_server.log/permissions.log）
//   - 全量汇总文件输出（full.log）
//   - 控制台彩色输出
...
//
// 架构：每个组件（Gateway/Channel/AgentServer/Permissions）创建独立的 zerolog.Logger 实例，
// 通过 GetLogger(component) 获取。每个 Logger 实例的 writer 同时写入组件日志文件、full.log 和控制台。
...
//	logger/
//	├── doc.go           # 包文档
//	├── logger.go        # 核心结构体、Option 模式、Setup/GetLogger/Close
