...
// ConfigFile 返回配置文件路径：WorkspaceDir()/config/config.yaml
func ConfigFile() string {
	return filepath.Join(WorkspaceDir(), "config", "config.yaml")
}

// EnvFile 返回环境变量文件路径：WorkspaceDir()/config/.env
func EnvFile() string {
	return filepath.Join(WorkspaceDir(), "config", ".env")
}
	...
	resolveOnce.Do(func() {
		...
		if dirExists(userConfigDir) {
			...
			if err != nil {
				...
				logger.Warn(logComponent).Err(err).Msg("未找到 resources 目录，降级为用户目录")
				resolvedPaths = &ResolvedPaths{
					ConfigDir:    userConfigDir,
					WorkspaceDir: userWorkspaceDir,
				}
			...
			logger.Info(logComponent).Str("resources", resDir).Msg("工作区未初始化，回退到 resources 目录")
			resolvedPaths = &ResolvedPaths{
				ConfigDir:    resDir,
