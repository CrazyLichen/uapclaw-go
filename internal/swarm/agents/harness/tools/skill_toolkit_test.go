...
// TestInstallSkill_SkillNet_轮询失败 验证 SkillNet 安装轮询失败（当前未实现）
func TestInstallSkill_SkillNet_轮询失败(t *testing.T) {
	tmpDir := t.TempDir()
	sm := skillpkg.NewSkillManager(tmpDir)
	tk := NewSkillToolkit(sm)
	...
	// SkillNet 安装尚未实现，job 会被设为 failed
	if toBool(result["success"]) {
		t.Errorf("应返回 success=false（SkillNet 未实现）, got %v", result)
	}
	detail := toString(result["detail"])
	if !strings.Contains(detail, "SkillNet") && !strings.Contains(detail, "尚未实现") {
		t.Errorf("detail 应包含 SkillNet 未实现信息, got %q", detail)
	}
}

// TestInstallSkill_SkillNet_安装失败 验证 SkillNet 安装失败（当前未实现）
