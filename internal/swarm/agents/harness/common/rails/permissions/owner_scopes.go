...
// Python: owner_scopes.PermissionContext
...
// Python: TOOL_PERMISSION_CONTEXT ContextVar
...
// Python: _severity = {"allow": 0, "ask": 1, "deny": 2}
...
// Python: setup_permission_context(request) 中构建 PermissionContext
...
//
// Python: _match_args() (owner_scopes.py L209-231)
func MatchArgs(pattern string, toolArgs map[string]any) (result bool) {
	defer func() {
		if rec := recover(); rec != nil {
			// Python: except Exception: return False
			result = false
		}
	}()
...
//
// Python: _resolve_owner_scope_level() (owner_scopes.py L172-206)
...
//
// Python: check_avatar_permission() (owner_scopes.py L96-169)
func CheckAvatarPermission(permCfg map[string]any, toolName string, toolArgs map[string]any, channelID string, sessionID string, principalUserID string) (string, error) {
	// Python: if perm_ctx is None or not perm_ctx.principal_user_id: return "deny"
	...
	// Python: owner_scopes = perm_cfg.get("owner_scopes")
	...
	logger.Info(logComponent).
		...
		Any("owner_scopes_keys", func() any {
			if ownerScopes != nil {
				keys := make([]string, 0, len(ownerScopes))
				for k := range ownerScopes {
					keys = append(keys, k)
				}
				return keys
			}
	...
	// Python: if not isinstance(owner_scopes, dict) or not owner_scopes: return "allow"
	...
	// Python: scope_cfg = (owner_scopes.get(cid) or {}).get(uid)
	...
	// Python: level = _resolve_owner_scope_level(scope_cfg, tool_name, tool_args)
	...
	// Python: engine.evaluate_global_policy_directly(tool_name, tool_args, include_external_directory=True)
		...
		defer func() {
			if rec := recover(); rec != nil {
				logger.Warn(logComponent).
					Str("tool_name", toolName).
					Any("recover", rec).
					Msg("[check_avatar_permission] evaluate_global_policy_directly 失败")
			}
		}()
	...
	// Python: severity 严格度比较
	finalLevel := level
	if level == "" {
		// Python: if level is None: 如果 global 是 allow 则 allow，否则 deny
		if globalLevelStr == "allow" {
			return "allow", nil
		}
		return "deny", nil
	}
...
//
// Python: persist_to_owner_scope() (owner_scopes.py L234-259)
func PersistToOwnerScope(toolName string, pattern string, channelID string, userID string, permCfg map[string]any, configPath string) bool {
	...
	// Python: user = ch.setdefault(user_id, {})
	...
	// Python: tools = user.setdefault("tools", {})
	...
	// Python: existing = tools.get(tool_name); if isinstance(existing, dict): existing["*"] = pattern; else: tools[tool_name] = pattern
...
// Scene 返回权限场景类型。
// Python: PermissionContext.scene
