 ...
 * 优先级：环境变量 > 已存在的用户工作区 ~/.uapclaw > 仓库根。
 */
function resolveProjectRootDir(): string {
  const envRoot = process.env.UAPCLAW_ROOT || process.env.UAPCLAW_PROJECT_ROOT
  if (envRoot) {
  ...
  if (home) {
    // 优先检查多实例环境变量
    const envWorkspace = process.env.UAPCLAW_DATA_DIR
    if (envWorkspace) {
      console.log('[file-api] 使用 UAPCLAW_DATA_DIR:', path.resolve(envWorkspace))
      return path.resolve(envWorkspace)
    }
    const userWorkspace = path.join(home, '.uapclaw')
            ...
            if (!fs.existsSync(fullPath)) {
              if (filePath.replace(/\\/g, '/') === 'agent/workspace/agent-data.json') {
                try {
                  const runResult = spawnSync(process.execPath, [generateAgentFoldersScriptPath], {
                    encoding: 'utf-8',
                    env: { ...process.env, UAPCLAW_ROOT: projectRootDir },
