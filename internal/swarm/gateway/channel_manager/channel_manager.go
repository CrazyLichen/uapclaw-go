...
// SetMessageHandler 设置入站和出站消息处理器。
//
// 用于 ChannelManager 和 MessageHandler 存在循环依赖的场景：
// 先创建 ChannelManager（inboundHandler/consumeProvider 为 nil），
// 再创建 MessageHandler，最后调用此方法回填。
func (cm *ChannelManager) SetMessageHandler(inboundHandler InboundMessageHandler, consumeProvider RobotMessageConsumer) {
	cm.mu.Lock()
	defer cm.mu.Unlock()

	cm.inboundHandler = inboundHandler
	cm.consumeProvider = consumeProvider
}
