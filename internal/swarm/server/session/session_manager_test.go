...
// TestNormalizeSessionID_空串 验证 NormalizeSessionID 空串返回 default
func TestNormalizeSessionID_空串(t *testing.T) {
	if got := NormalizeSessionID(""); got != "default" {
		t.Errorf("NormalizeSessionID(\"\") = %q, want %q", got, "default")
	}
}

// TestNormalizeSessionID_非空 验证 NormalizeSessionID 非空原样返回
func TestNormalizeSessionID_非空(t *testing.T) {
	if got := NormalizeSessionID("sess_abc"); got != "sess_abc" {
		t.Errorf("NormalizeSessionID(\"sess_abc\") = %q, want %q", got, "sess_abc")
	}
}
