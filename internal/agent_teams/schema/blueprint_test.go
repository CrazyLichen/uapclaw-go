...
func TestNewTeamModelConfig(t *testing.T) {
	...
	if cfg.ModelRequestConfig == nil { t.Error("ModelRequestConfig 不应为 nil") }
	if cfg.ModelClientConfig.Timeout != 60.0 { t.Errorf("期望 Timeout=60.0, 实际=%f", cfg.ModelClientConfig.Timeout) }
}

// TestTeamModelConfig_Build_留桩 测试 Build 留桩
func TestTeamModelConfig_Build_留桩(t *testing.T) {
	r, err := NewTeamModelConfig().Build()
	if r != nil || err != nil { t.Errorf("期望 (nil, nil), 实际=(%v, %v)", r, err) }
}
...
func TestNewLeaderSpec(t *testing.T) {
	...
	s := NewLeaderSpec()
	if s.MemberName != "team_leader" { t.Errorf("期望 'team_leader', 实际=%q", s.MemberName) }
	if s.DisplayName != "Team Leader" { t.Errorf("期望 'Team Leader', 实际=%q", s.DisplayName) }
}
...
func TestStorageSpec_Build_内置(t *testing.T) {
	...
	if !ok { t.Fatal("期望 DatabaseConfig") }
	if dbCfg.DBType != database.DatabaseTypeSQLite { t.Errorf("期望 sqlite, 实际=%q", dbCfg.DBType) }
}
...
func TestEnsureBuiltinInfraRegistered(t *testing.T) {
	...
	for _, n := range []string{"inprocess", "pyzmq"} {
		if _, ok := transportRegistry[n]; !ok { t.Errorf("传输 %q 未注册", n) }
	}
	for _, n := range []string{"sqlite", "postgresql", "mysql", "memory"} {
		if _, ok := storageRegistry[n]; !ok { t.Errorf("存储 %q 未注册", n) }
	}
}

// TestTeamAgentSpec_Validate_正常 测试正常验证
func TestTeamAgentSpec_Validate_正常(t *testing.T) {
	if err := NewTeamAgentSpec().Validate(); err != nil { t.Errorf("报错: %v", err) }
}
...
func TestTeamAgentSpec_Validate_PoolRouter互斥(t *testing.T) {
	...
	if err := s.Validate(); err == nil { t.Error("期望互斥报错") }
}
...
func TestTeamAgentSpec_DefaultTransportForSpawnMode_inprocess(t *testing.T) {
	...
	_ = s.Validate()
	if s.Transport == nil || s.Transport.Type != "inprocess" { t.Error("期望自动填充 inprocess") }
}
...
func TestTeamAgentSpec_DefaultTransportForSpawnMode_process(t *testing.T) {
	...
	s.SpawnMode = "process"
	_ = s.Validate()
	if s.Transport != nil { t.Error("期望不填充") }
}
...
func TestTeamAgentSpec_ValidateReservedNames_正常(t *testing.T) {
	...
	if err := s.validateReservedNames(); err != nil { t.Errorf("报错: %v", err) }
}
...
func TestTeamAgentSpec_ValidateReservedNames_保留名(t *testing.T) {
	...
	if err := s.validateReservedNames(); err == nil { t.Error("期望报错") }
}
...
func TestTeamAgentSpec_ValidateHittConsistency_正常(t *testing.T) {
	if err := NewTeamAgentSpec().validateHittConsistency(); err != nil { t.Errorf("报错: %v", err) }
}
...
func TestTeamAgentSpec_ValidateHittConsistency_未启用(t *testing.T) {
	s := NewTeamAgentSpec()
	s.PredefinedMembers = []TeamMemberSpec{{MemberName: "human_agent", RoleType: TeamRoleHumanAgent}}
	if err := s.validateHittConsistency(); err == nil { t.Error("期望报错") }
}
...
func TestTeamAgentSpec_ValidateLeaderModelResolved_有模型(t *testing.T) {
	...
	if err := ValidateLeaderModelResolved(a, nil, ts); err != nil { t.Errorf("报错: %v", err) }
}
...
func TestTeamAgentSpec_ValidateLeaderModelResolved_无模型(t *testing.T) {
	a := NewDeepAgentSpec()
	ts := TeamSpec{ModelPool: []map[string]any{{"model_name": "test"}}}
	if err := ValidateLeaderModelResolved(a, nil, ts); err == nil { t.Error("期望报错") }
}
...
func TestTeamAgentSpec_ResolveDBConfig_存储配置(t *testing.T) {
	...
	if !ok { t.Fatal("期望 MemoryDatabaseConfig") }
	if memCfg.DBType != database.DatabaseTypeMemory { t.Errorf("期望 memory, 实际=%q", memCfg.DBType) }
}

// TestTeamAgentSpec_Build_留桩 测试 Build 留桩
func TestTeamAgentSpec_Build_留桩(t *testing.T) {
	r, err := NewTeamAgentSpec().Build()
	if r != nil || err != nil { t.Errorf("期望 (nil, nil), 实际=(%v, %v)", r, err) }
}
...
func TestNewDeepAgentSpec(t *testing.T) {
	s := NewDeepAgentSpec()
	if s.MaxIterations != 15 { t.Errorf("期望 15, 实际=%d", s.MaxIterations) }
	if !s.AutoCreateWorkspace { t.Error("期望 true") }
	if s.CompletionTimeout != 600.0 { t.Errorf("期望 600.0, 实际=%f", s.CompletionTimeout) }
}
...
func TestDeepAgentSpec_JSON序列化(t *testing.T) {
	...
	if err != nil { t.Fatalf("序列化失败: %v", err) }
	var d DeepAgentSpec
	if err := json.Unmarshal(data, &d); err != nil { t.Fatalf("反序列化失败: %v", err) }
	if d.SystemPrompt != s.SystemPrompt { t.Errorf("不匹配") }
	if d.MaxIterations != s.MaxIterations { t.Errorf("不匹配") }
}
...
func TestRegisterTransport(t *testing.T) {
	...
	if err != nil { t.Fatalf("报错: %v", err) }
	if cfg.Backend != nt { t.Errorf("不匹配") }
}
...
func TestRegisterStorage(t *testing.T) {
	...
	memCfg, ok := cfg.(database.MemoryDatabaseConfig)
	if !ok { t.Fatal("期望 MemoryDatabaseConfig") }
	if memCfg.DBType != database.DatabaseTypeMemory { t.Errorf("不匹配") }
}
...
func TestWorkspaceSpec_JSON序列化(t *testing.T) {
	...
	var d WorkspaceSpec
	if err := json.Unmarshal(data, &d); err != nil { t.Fatalf("反序列化失败: %v", err) }
	if d.RootPath != s.RootPath { t.Errorf("不匹配") }
}
...
func TestVisionModelSpec_JSON序列化(t *testing.T) {
	...
	if err := json.Unmarshal(data, &d); err != nil { t.Fatalf("反序列化失败: %v", err) }
	if d.Model != s.Model { t.Errorf("不匹配") }
}
