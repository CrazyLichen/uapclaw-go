...
// Python: HumanAgentNotEnabledError (openjiuwen/agent_teams/interaction/human_agent_inbox.py)
...
// Python: HumanAgentInbox (openjiuwen/agent_teams/interaction/human_agent_inbox.py)
//
// 路由规则：
...
// Python: HumanAgentInbox.__init__(team, message_manager, *, agent_lookup, on_inbound)
...
// Python: HumanAgentInbox.send(body, to, sender)
//
// Python 执行步骤：
...
func (h *HumanAgentInbox) Send(body string, to *string, sender *string) (*DeliverResult, error) {
	...
	// Python 步骤 3: if to is None: return await self._drive_agent(...)
	if to == nil {
		return h.driveAgent(body, resolvedSender)
	}

	// Python 步骤 4: if to in BROADCAST_TARGETS: broadcast
	if BroadcastTargets[*to] {
		// Python: msg_id = await self._mm.broadcast_message(content=body, from_member_name=resolved_sender)
...
// Python: HumanAgentInbox.on_inbound (property)
...
// Python: HumanAgentInbox._resolve_sender(sender)
//
// Python 执行步骤：
...
func (h *HumanAgentInbox) resolveSender(sender *string) (string, error) {
	...
	// Python 步骤 2: if not names: raise HumanAgentNotEnabledError
	if len(names) == 0 {
		return "", &HumanAgentNotEnabledError{}
	}

	// Python 步骤 3: if sender is None
	if sender == nil {
		// Python 步骤 3a: if HUMAN_AGENT_MEMBER_NAME in names: return HUMAN_AGENT_MEMBER_NAME
		for _, n := range names {
			if n == agentteams.HumanAgentMemberName {
				return n, nil
			}
		}
		// Python 步骤 3b: return sorted(names)[0]
	...
	// Python 步骤 4: if sender not in names: raise UnknownHumanAgentError
	for _, n := range names {
		if n == *sender {
			return *sender, nil
		}
...
// Python: HumanAgentInbox._drive_agent(body, sender)
//
// Python 执行步骤：
...
// Python: HumanAgentInbox._member_exists(name)
