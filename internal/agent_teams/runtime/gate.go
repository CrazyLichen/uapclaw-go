...
// Python: AdmissionTicket (openjiuwen/agent_teams/runtime/gate.py)
//
...
// Python: InteractGate (openjiuwen/agent_teams/runtime/gate.py)
//
...
//	OPEN    --关闭并排空()--> 关闭中 --(inflight==0)--> 已排空
//	状态转换: 关闭中 --Admit()-------->      nil (被拒绝)
...
// Closed 门控是否已关闭。
// Python: InteractGate.closed (property)
...
// Inflight 当前飞行中的载荷数。
// Python: InteractGate.inflight (property)
...
// ConsumeDone 标记载荷已消费。
// Python: InteractGate.consume_done(ticket)
//
// Python 执行步骤：
...
func (g *InteractGate) ConsumeDone(ticket *AdmissionTicket) {
	...
	// Python 步骤 4
	g.inflight--
	// Python 步骤 5: if self._inflight == 0: self._drained.set()
	if g.inflight == 0 {
		close(g.drained)
	}
}
...
// Python: InteractGate.close_and_drain()
//
// Python 执行步骤：
//  1. async with self._lock:
//  2. self._closed.set()
...
func (g *InteractGate) CloseAndDrain(ctx context.Context) error {
	...
	// Python 步骤 5: await self._drained.wait()
	select {
	case <-drainedCh:
		return nil
	case <-ctx.Done():
