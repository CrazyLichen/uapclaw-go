...
//
// Python: parse_json(output, header=None)
//
...
func ParseJSON(output string, header ...string) map[string]any {
	...
	if len(header) > 0 && header[0] != "" {
		// Python: json_idx = output.find(f'{{"{header}":')
		jsonIdx = strings.Index(output, `{"`+header[0]+`":`)
		if jsonIdx == -1 {
			// Python: json_idx = output.find(f'{{\n"{header}":')
			jsonIdx = strings.Index(output, "{\n\""+header[0]+"\":")
		}
	}

	if jsonIdx == -1 {
		// Python: json_idx = output.find('{\n')
		jsonIdx = strings.Index(output, "{\n")
	}
	if jsonIdx == -1 {
		// Python: json_idx = output.find('{')
	...
	// Python: json_end_idx = output.rfind('}')
	jsonEndIdx := strings.LastIndex(output, "}")
	if jsonEndIdx != -1 {
		// Python: json_end_idx = json_end_idx + 1
		jsonEndIdx++
	}
	...
	// Python: output = output[json_idx:json_end_idx].strip()
	extracted := strings.TrimSpace(output[jsonIdx:jsonEndIdx])

	var result map[string]any
	// Python: output_json = json.loads(output)
	if err := json.Unmarshal([]byte(extracted), &result); err != nil {
		// Python: output_json = ast.literal_eval(output)
		// 1. 单引号 → 双引号
...
//
// Python: format_prompt_llama(system_prompt, user_prompt)
//
