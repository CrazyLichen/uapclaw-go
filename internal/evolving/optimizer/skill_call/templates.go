...
// 一比一复刻 Python templates.py SKILL_EXPERIENCE_GENERATE_PROMPT_CN。
//
// 注意：Python 使用 .format() 替换占位符，{{ 在 .format() 中表示字面量 {。
// Go 中直接使用 {placeholder} 格式，后续通过 formatTemplate(strings.ReplaceAll) 替换。
// Python 中的 {{ 转为 Go 中的 {（Python {{ 在 .format() 中是 { 的转义，Go 不需要）。
var SkillExperienceGeneratePromptCN = strings.Join([]string{
...
// 一比一复刻 Python templates.py SKILL_EXPERIENCE_GENERATE_PROMPT_EN。
var SkillExperienceGeneratePromptEN = strings.Join([]string{
...
// JSONFixPrompt 个体 JSON 修复提示词。
// 一比一复刻 Python templates.py JSON_FIX_PROMPT。
var JSONFixPrompt = strings.Join([]string{
...
// JSONFixPromptStrict 个体 JSON 严格修复提示词。
// 一比一复刻 Python templates.py JSON_FIX_PROMPT_STRICT。
var JSONFixPromptStrict = strings.Join([]string{
...
// UserPatchPromptCN 团队用户 patch 提示词（中文）。
// 一比一复刻 Python templates.py USER_PATCH_PROMPT_CN。
var UserPatchPromptCN = strings.Join([]string{
...
// UserPatchPromptEN 团队用户 patch 提示词（英文）。
// 一比一复刻 Python templates.py USER_PATCH_PROMPT_EN。
var UserPatchPromptEN = strings.Join([]string{
...
// TrajectoryPatchPromptCN 团队轨迹 patch 提示词（中文）。
// 一比一复刻 Python templates.py TRAJECTORY_PATCH_PROMPT_CN。
var TrajectoryPatchPromptCN = strings.Join([]string{
...
// TrajectoryPatchPromptEN 团队轨迹 patch 提示词（英文）。
// 一比一复刻 Python templates.py TRAJECTORY_PATCH_PROMPT_EN。
var TrajectoryPatchPromptEN = strings.Join([]string{
...
// TeamExperienceGeneratePromptCN 团队经验生成提示词（中文）。
// 一比一复刻 Python templates.py TEAM_EXPERIENCE_GENERATE_PROMPT_CN。
var TeamExperienceGeneratePromptCN = strings.Join([]string{
...
// TeamExperienceGeneratePromptEN 团队经验生成提示词（英文）。
// 一比一复刻 Python templates.py TEAM_EXPERIENCE_GENERATE_PROMPT_EN。
var TeamExperienceGeneratePromptEN = strings.Join([]string{
...
// TeamJSONFixPrompt 团队 JSON 修复提示词。
// 一比一复刻 Python templates.py TEAM_JSON_FIX_PROMPT。
var TeamJSONFixPrompt = strings.Join([]string{
	"你上次的输出不是合法 JSON，请修复并重新输出。",
	"只输出修复后的 JSON 数组，不要任何解释文字。",
	"",
	"## 解析错误",
...
// TeamJSONFixPromptStrict 团队 JSON 严格修复提示词。
// 一比一复刻 Python templates.py TEAM_JSON_FIX_PROMPT_STRICT。
var TeamJSONFixPromptStrict = strings.Join([]string{
	"你的 JSON 输出多次解析失败。请完全重新生成。",
	"",
	"## 解析错误",
	"{parse_error}",
...
// SkillExperienceGeneratePrompt 个体经验生成提示词映射（语言 → 模板）。
// 一比一复刻 Python templates.py SKILL_EXPERIENCE_GENERATE_PROMPT。
var SkillExperienceGeneratePrompt = map[string]string{
	"cn": SkillExperienceGeneratePromptCN,
	"en": SkillExperienceGeneratePromptEN,
}

// UserPatchPrompt 团队用户 patch 提示词映射（语言 → 模板）。
// 一比一复刻 Python templates.py USER_PATCH_PROMPT。
var UserPatchPrompt = map[string]string{
	"cn": UserPatchPromptCN,
	"en": UserPatchPromptEN,
}

// TrajectoryPatchPrompt 团队轨迹 patch 提示词映射（语言 → 模板）。
// 一比一复刻 Python templates.py TRAJECTORY_PATCH_PROMPT。
var TrajectoryPatchPrompt = map[string]string{
	"cn": TrajectoryPatchPromptCN,
	"en": TrajectoryPatchPromptEN,
}

// TeamExperienceGeneratePrompt 团队经验生成提示词映射（语言 → 模板）。
// 一比一复刻 Python templates.py TEAM_EXPERIENCE_GENERATE_PROMPT。
var TeamExperienceGeneratePrompt = map[string]string{
