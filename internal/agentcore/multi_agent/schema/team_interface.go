...
//
// Python: openjiuwen/core/multi_agent/team.py (BaseTeam)
//
// 设计原则：
//   - Card 是必填项（定义团队身份）
...
type BaseTeam interface {
	...
	// Invoke 非流式调用团队。
	//
	// Python: BaseTeam.invoke(message, session)
	Invoke(ctx context.Context, inputs map[string]any, opts ...TeamOption) (any, error)

	// Stream 流式调用团队。
	//
	// Python: BaseTeam.stream(message, session) -> AsyncIterator
	...
	// 其他 Team 实现忽略 opts。
	//
	// Python: BaseTeam.add_agent(card, provider, parent_agent_id=None) -> self
	...
	//
	// Python: BaseTeam.remove_agent(agent) -> self
...
// TeamOptions 团队调用选项。
//
// Python: BaseTeam 各方法的可选参数（session、session_id、timeout、stream_modes）
type TeamOptions struct {
	// Session 团队会话（可选）
	//
	// Python: invoke(message, session) 的 session 参数
	Session *session.AgentTeamSession
	// SessionID 会话标识（可选）
	//
	// Python: send/publish 的 session_id 参数
	SessionID string
	// Timeout 响应超时秒数（可选）
	//
	// Python: send 的 timeout 参数
	Timeout float64
	// StreamModes 流式输出模式（可选）
	//
	// Python: stream 的 stream_modes 参数
...
//
// Python: AgentTeamProvider = Callable[[TeamCard], Awaitable[BaseTeam]] | Callable[[TeamCard], BaseTeam]
...
//
// Python: AgentProvider = Callable[[AgentCard], Awaitable[BaseAgent]] | Callable[[AgentCard], BaseAgent]
...
//
// Python: TeamConfig(max_agents=10, max_concurrent_messages=100, message_timeout=30.0)
...
//
// Python: TeamConfig.configure_max_agents(max_agents) -> self
...
//
// Python: TeamConfig.configure_timeout(timeout) -> self
...
//
// Python: TeamConfig.configure_concurrency(max_concurrent) -> self
