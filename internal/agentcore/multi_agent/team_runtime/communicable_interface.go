...
type Communicable interface {
	...
	Send(ctx context.Context, message any, recipient string, opts ...maschema.TeamOption) (any, error)
	// Publish Pub-Sub 发布消息到指定主题，发后即忘。
	Publish(ctx context.Context, message any, topicID string, opts ...maschema.TeamOption) error
	// Subscribe 订阅主题。
