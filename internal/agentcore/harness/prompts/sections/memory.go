...
const (
	memoryPromptCNReadOnly = `# 持久化存储体系（只读模式）
...
`
	...
	memoryPromptENReadOnly = `# Persistent Storage System (Read-Only Mode)
...
`
	...
	memoryPromptEN = `# Persistent Storage System
...
`
	...
	memoryMgmtPromptEN = `### Storage Management Guidelines
...
`
	...
	memoryInactivePromptCN = `## 持久化存储体系（被动模式）
...
`
	...
	memoryInactivePromptEN = `## Persistent Storage System (Passive Mode)
...
func BuildMemorySection(mode string, todayDate string, lang string) saprompt.PromptSection {
	...
	case "read_only":
		if lang == "en" {
			parts = append(parts, memoryPromptENReadOnly)
		} else {
			parts = append(parts, memoryPromptCNReadOnly)
		}
	case "inactive":
		if lang == "en" {
			parts = append(parts, memoryInactivePromptEN)
		} else {
	...
	default: // "proactive"
		if lang == "en" {
			...
			parts = append(parts, strings.ReplaceAll(memoryDatePromptEN, "{today_date}", todayDate))
		} else {
			parts = append(parts, memoryPromptCN)
			parts = append(parts, memoryMgmtPromptCN)
			parts = append(parts, strings.ReplaceAll(memoryDatePromptCN, "{today_date}", todayDate))
		}
	}
...
// BuildMemoryMgmtPrompt 构建存储管理规范提示词
func BuildMemoryMgmtPrompt(lang string) string {
	if lang == "en" {
		return memoryMgmtPromptEN
	}
	return memoryMgmtPromptCN
}

// BuildMemoryDatePrompt 构建日期提示词
func BuildMemoryDatePrompt(todayDate string, lang string) string {
	if lang == "en" {
		return strings.ReplaceAll(memoryDatePromptEN, "{today_date}", todayDate)
	}
