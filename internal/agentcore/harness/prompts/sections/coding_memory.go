...
const (
	codingMemoryReadOnlyCN = `# coding memory（只读）
位于 ` + "`{memory_dir}`" + `。用 coding_memory_read 读取。不允许写入。
`

	codingMemoryReadOnlyEN = `# coding memory (read-only)
At ` + "`{memory_dir}`" + `. Use coding_memory_read to read. No writing allowed.
`

	codingMemoryPromptCN = `# coding memory
...
`
	...
	codingMemoryPromptEN = `# coding memory
...
func BuildCodingMemorySection(memoryDir string, readOnly bool, lang string) saprompt.PromptSection {
	...
	if readOnly {
		if lang == "en" {
			template = codingMemoryReadOnlyEN
		} else {
			template = codingMemoryReadOnlyCN
		}
	} else {
		if lang == "en" {
			template = codingMemoryPromptEN
		} else {
			template = codingMemoryPromptCN
		}
	}

	content := strings.ReplaceAll(template, "{memory_dir}", memoryDir)
