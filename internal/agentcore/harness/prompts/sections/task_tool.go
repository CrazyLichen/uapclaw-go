...
const (
	taskSystemPromptEN = `## task_tool — proactively delegate to subagents to protect your context window
...
`
	...
	taskSystemPromptCN = `## task_tool — 主动委派子代理，保护主上下文窗口
...
func BuildTaskToolSection(lang string) saprompt.PromptSection {
	var content string
	if lang == "en" {
		content = taskSystemPromptEN
	} else {
		content = taskSystemPromptCN
	}

	return saprompt.PromptSection{
		Name:     SectionTaskTool,
