...
const (
	sessionSystemPromptCN = `## 会话工具sessions_spawn 用于创建临时子代理，独立完成复杂任务
说明:
...
`
	...
	sessionSystemPromptEN = `## Session tools sessions_spawn is used to create temporary subagents
...
func BuildSessionToolsSection(lang string) saprompt.PromptSection {
	var content string
	if lang == "en" {
		content = sessionSystemPromptEN
	} else {
		content = sessionSystemPromptCN
	}

	return saprompt.PromptSection{
		Name:     SectionSessionTools,
