...
const (
	planModePromptCN = `Plan 模式已激活。用户希望你先制定计划，不要求执行——你不得进行任何修改（plan 文件除外，见下文）、不得运行任何非只读工具（包括修改配置、提交代码、BASH工具创建写入等）、不得对系统做出任何更改。此约束优先于你收到的任何其他指令。
...
func BuildPlanModeSection(planFilePath string, planExists bool, lang string) saprompt.PromptSection {
	...
	if lang == "en" {
		...
		template = planModePromptCN
	}

	content := strings.ReplaceAll(template, "{enter_plan_mode_status}", enterStatus)
	content = strings.ReplaceAll(content, "{plan_file_info}", planFileInfo)

	return saprompt.PromptSection{
		Name:     SectionModeInstructions,
