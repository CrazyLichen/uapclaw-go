...
//
// 对齐 Python: build_permission_interrupt_rail(permissions, llm, model_name, engine, host, workspace_root) -> PermissionInterruptRail | None (factory.py)
...
func BuildPermissionInterruptRail(
	...
	// 对齐 Python: if not isinstance(permissions, dict) or not permissions.get("enabled", False)
	enabled := false
	...
	// 对齐 Python: h = host or ToolPermissionHost()
	h := host
	if h == nil {
		h = &ToolPermissionHost{}
	}

	// 对齐 Python: if h.resolve_workspace_dir is None and workspace_root is not None
