...
//
// 本包实现了 9.24 的 P1 契约层和 P2（EvolutionRail 基类 + TrajectoryRail），
// 为 P3（SkillEvolutionRail）、P4（TeamSkillEvolutionRail）提供基础类型和基类。
//
// 核心功能：
...
//   - EvolutionRail 基类：嵌入 DeepAgentRail，4 个 final 回调自动完成轨迹收集，9 个扩展点通过 EvolutionExtension 接口实现多态分派
...
//	├── extension.go          # EvolutionExtension 接口（9 方法）+ noOpExtension 默认实现 + EvolutionTriggerPoint 枚举
