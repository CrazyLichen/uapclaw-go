...
type EvolutionExtension interface {
	...
	// Python: _on_after_model_call(ctx)
	...
	// Python: _on_after_tool_call(ctx)
	...
	// Python: _on_after_evolution_triggered(trajectory, ctx)
	...
	// Python: _allow_evolution_trigger(trigger_point, ctx) -> bool
	...
	// Python: _snapshot_for_evolution(trajectory, ctx) -> Optional[dict]
	...
	// Python: run_evolution(trajectory, ctx=None, *, snapshot=None)
	...
	// Python: EvolutionExtension.get_evolution_total_timeout_secs()
	GetEvolutionTotalTimeoutSecs() float64
}
...
// GetEvolutionTotalTimeoutSecs 空实现，返回 0 表示不限时。
// Python: EvolutionRail._get_evolution_total_timeout_secs() -> None
func (noOpExtension) GetEvolutionTotalTimeoutSecs() float64 { return 0 }
