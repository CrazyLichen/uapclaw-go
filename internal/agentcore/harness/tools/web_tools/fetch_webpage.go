...
// Python: WebFetchWebpageTool.invoke inputs (web_tools.py L1598-1603)
...
var (
	...
	// Python: _CHARSET_HEADER_RE (web_tools.py L33)
	charsetHeaderRe = regexp.MustCompile(`(?i)charset=([^\s;]+)`)
	// charsetMetaRe HTML meta charset 正则
	// Python: _CHARSET_META_RE (web_tools.py L34-37)
...
// Python: WebFetchWebpageTool.__init__ + invoke (web_tools.py L1384-1632)
func NewWebFetchWebpageTool(language, agentID string) tool.Tool {
	...
	fn := func(ctx context.Context, input FetchWebpageInput, opts ...tool.ToolOption) (map[string]any, error) {
		// Python: WebFetchWebpageTool.invoke (web_tools.py L1596-1627)
...
// Python: WebFetchWebpageTool._fetch_webpage_sync() (web_tools.py L1555-1576)
func fetchWebpage(rawURL string, timeoutSeconds int) (map[string]any, error) {
	...
	// Python: L1558-1559 — 401/403/429 降级到 Jina
...
// Python: WebFetchWebpageTool._fetch_via_jina_reader_sync() (web_tools.py L1470-1480)
...
// Python: WebFetchWebpageTool._decode_response_text() (web_tools.py L1422-1467)
...
// Python: WebFetchWebpageTool._extract_declared_charset() (web_tools.py L1391-1405)
func extractDeclaredCharset(resp *httpResponse) string {
	// Python: L1393-1396 — Content-Type 头
	contentType := resp.headers.Get("Content-Type")
	if match := charsetHeaderRe.FindStringSubmatch(contentType); len(match) > 1 {
		return strings.Trim(match[1], ` "'`)
	}

	// Python: L1398-1404 — HTML meta 标签
	headBytes := resp.body
...
// Python: WebFetchWebpageTool._score_decoded_text() (web_tools.py L1408-1419)
...
func decodeBytes(data []byte, encoding string) (string, error) {
	...
	if err != nil {
		return "", err // Python: strict 模式，解码失败时抛出异常
	}
	text := string(decoded)
	// 去除 UTF-8 BOM（\xEF\xBB\xBF），对齐 Python utf-8-sig 行为
	text = strings.TrimPrefix(text, "\xEF\xBB\xBF")
