...
const (
	// comparativeMemoryPrompt 对比提示词
	comparativeMemoryPrompt = `You are an expert AI analyst comparing higher-scoring and lower-scoring step sequences to extract performance insights.
...
` + "```"
	...
	// successMemoryPrompt 成功提取提示词
	successMemoryPrompt = `You are an expert AI analyst reviewing successful step sequences from an AI agent execution.
...
` + "```"
	...
	// failureMemoryPrompt 失败提取提示词
	failureMemoryPrompt = `You are an expert AI analyst reviewing failed step sequences from an AI agent execution.
...
` + "```"
	...
	// comparativeAllMemoryPrompt 全量对比提示词
	comparativeAllMemoryPrompt = `You are an expert AI analyst comparing multiple step sequences which might be successful or failed to extract differential insights.
...
` + "```"
	...
	// memoryValidationPrompt 记忆校验提示词
	memoryValidationPrompt = `You are an expert AI analyst tasked with validating the quality and usefulness of extracted step-level task memories.
