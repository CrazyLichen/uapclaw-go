...
// reflectorPromptData Reflector prompt 数据（含 Ground Truth）
...
// reflectorScalingPromptData Reflector Scaling prompt 数据（含 Ground Truth）
...
// curatorPromptData Curator prompt 数据
type curatorPromptData struct {
	// QuestionContext 任务上下文
...
// curatorScalingPromptData Curator Scaling prompt 数据
type curatorScalingPromptData struct {
	// QuestionContext 任务上下文
	QuestionContext string
	// Playbook 当前 Playbook
	Playbook string
	// Trajectories 多条生成的尝试
	Trajectories string
	// Reflection 当前反思
	Reflection string
}
...
// 一比一复刻 Python ACE_REFLECTOR_PROMPT。
const aceReflectorPrompt = `You are an expert reflection agent and educator. Your job is to diagnose the current trajectory: identify what went wrong (or could be better), grounded in execution feedback, API usage, unit test report, and ground truth when applicable.
...
// 一比一复刻 Python ACE_REFLECTOR_NOGT_PROMPT。
const aceReflectorNoGTPrompt = `You are an expert reflection agent and educator. Your job is to diagnose the current trajectory: identify what went wrong (or could be better), grounded in execution feedback and API usage.
...
const aceCuratorPrompt = `You are a master curator of knowledge. Your job is to identify what new insights should be added to an existing playbook based on a reflection from a previous attempt.
...
const aceReflectorScalingNoGTPrompt = `You are an expert reflection agent and educator. You will be given a user query and multiple trajectories showing how an agent attempted the task. Some trajectories may be successful, and others may have failed.
...
const aceCuratorScalingPrompt = `You are a master curator of knowledge. Your job is to identify what new insights should be added to an existing playbook based on a reflection from a previous attempt.
...
// NewACEPrompt 构造 ACEPrompt 实例，解析 6 个 prompt 模板。
