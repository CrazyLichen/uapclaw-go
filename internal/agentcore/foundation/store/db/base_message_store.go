...
type BaseMessageStore interface {
	...
	//
	// Python: BaseMessageStore.get_messages(message_filter, limit, order_by, order_direction)
	...
	// 0 表示无迁移操作，1+ 表示实际版本号。
	//
	// Python: BaseMessageStore.get_schema_version() -> int | None
	GetSchemaVersion(ctx context.Context) (int32, error)

	// SetSchemaVersion 设置 schema 版本号。
	//
	// Python: BaseMessageStore.set_schema_version(version)
	SetSchemaVersion(ctx context.Context, version int32) error
}

// MessageMetadata 消息元数据。
//
// Python: openjiuwen/core/foundation/store/base_message_store.py (MessageMetadata)
...
type MessageFilter struct {
	...
	// Python: 行为差异：Python session_id=None 时走 WHERE session_id IS NULL，
