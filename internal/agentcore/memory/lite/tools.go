...
func InitMemoryManagerAsync(ctx context.Context, ws *workspace.Workspace, agentID string, embeddingConfig *embedding.EmbeddingConfig, sysOp sysop.SysOperation) (MemoryIndexManager, error) {
	if !IsMemoryEnabled() {
		logger.Info(logComponent).Msg("记忆系统已禁用")
		return nil, nil
	}
	memoryDir := ""
	...
	logger.Info(logComponent).Str("memory_dir", memoryDir).Msg("记忆管理器已初始化")
	return mgr, nil
}

// InitCodingMemoryManagerAsync 初始化编程记忆管理器。对齐 Python coding_memory_tools.init_memory_manager_async
func InitCodingMemoryManagerAsync(ctx context.Context, ws *workspace.Workspace, agentID string, embeddingConfig *embedding.EmbeddingConfig, sysOp sysop.SysOperation, llmModel *llm.Model) (MemoryIndexManager, error) {
	if !IsMemoryEnabled() {
		logger.Info(logComponent).Msg("记忆系统已禁用")
		return nil, nil
	}
	...
	mgr, err := GetMemoryIndexManager(params)
	if err != nil {
		logger.Error(logComponent).Err(err).Msg("初始化编程记忆管理器失败")
