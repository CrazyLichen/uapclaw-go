...
//
// Python: openjiuwen/core/memory/manage/index/variable_manager.py (VariableManager)
...
const (
	...
	// Python: VariableManager.SEPARATOR = "/"
	separator = "/"
	// userVarPrefix 用户级变量前缀
	// Python: VariableManager.USER_VAR_PREFIX = "user_var"
	userVarPrefix = "user_var"
	// sessionVarPrefix 会话级变量前缀
	// Python: VariableManager.SESSION_VAR_PREFIX = "session_var"
...
var (
	...
	// Python: VariableManager.LEGACY_PREFIXES: List[str] = []
...
func (m *VariableManager) AddMemories(ctx context.Context, userID string, scopeID string,
	...
	for memType, units := range memories {
		...
		for _, unit := range units {
			...
			if !ok {
				// Python: memory_logger.warning("mem_unit is not a VariableUnit", ...)
			...
			// Python: key, value = self._make_variable_pairs(user_id, False, scope_id, unit.variable_name, None, unit.variable_mem, None)
...
func (m *VariableManager) DeleteByUserID(ctx context.Context, userID string, scopeID string, opts ...MemoryOption) (bool, error) {
	...
	// Python: await self.kv_store.delete_by_prefix(user_prefix)
	...
	// Python: await self.kv_store.delete_by_prefix(session_prefix)
...
// 先查询变量是否存在，存在时更新。
//
// Python: VariableManager.update_user_variable
func (m *VariableManager) UpdateUserVariable(ctx context.Context, userID string, scopeID string, varName string, varMem string) error {
	...
	// Python: existing_variable = await self.query_variable(user_id=user_id, scope_id=scope_id, name=var_name)
	existing, err := m.QueryVariable(ctx, userID, scopeID, varName, "")
	if err != nil {
		return err
	}
	// Python: if not VariableManager._check_exist(existing_variable, var_name): return
	if !checkExist(existing, varName) {
		return nil
	}

	// Python: key, value = self._make_variable_pairs(usr_id=user_id, for_deletion=False, scope_id=scope_id, var_name=var_name, user_var_value=var_mem)
...
//
// Python: VariableManager.delete_user_variable
func (m *VariableManager) DeleteUserVariable(ctx context.Context, userID string, scopeID string, varName string) error {
	...
	// Python: key, _ = self._make_variable_pairs(usr_id=user_id, for_deletion=False, scope_id=scope_id, var_name=var_name)
...
//
// Python: VariableManager.query_variable
func (m *VariableManager) QueryVariable(ctx context.Context, userID string, scopeID string, name string, sessionID string) (map[string]string, error) {
	// Python: self._check_user_and_scope_id(user_id, scope_id, "Search")
	m.checkUserAndScopeID(userID, scopeID, "Search")

	// Python: if not name or not name.strip():
	if strings.TrimSpace(name) == "" {
		// Python: prefix_str = f"{self.USER_VAR_PREFIX}{self.SEPARATOR}{user_id}{self.SEPARATOR}{scope_id}{self.SEPARATOR}"
		prefixStr := fmt.Sprintf("%s%s%s%s%s%s", userVarPrefix, separator, userID, separator, scopeID, separator)
		// Python: kv_ret = await self.kv_store.get_by_prefix(prefix_str)
		kvRet, err := m.kvStore.GetByPrefix(ctx, prefixStr)
		if err != nil {
			return nil, err
		}
		// Python: result = {}; for k, v in kv_ret.items(): v = self._codec.decode(v); result[k.split(f"{self.SEPARATOR}")[-1]] = v
		result := make(map[string]string)
		for k, v := range kvRet {
			decoded := m.aesCodec.Decode(string(v))
			// Python: k.split(f"{self.SEPARATOR}")[-1]
	...
	if sessionID != "" {
		...
		// Python: key = f"{self.USER_VAR_PREFIX}{self.SEPARATOR}{user_id}{self.SEPARATOR}{scope_id}{self.SEPARATOR}{name}"
		key = fmt.Sprintf("%s%s%s%s%s%s%s", userVarPrefix, separator, userID, separator, scopeID, separator, name)
	}

	// Python: kv_ret = await self.kv_store.get(key); kv_ret = self._codec.decode(kv_ret); return {name: kv_ret}
...
//
// Python: VariableManager._make_variable_pairs
func (m *VariableManager) makeVariablePairs(usrID string, forDeletion bool, scopeID string, varName string, sessionID string, userVarValue string, sessionVarValue string) (string, []byte) {
	...
	// Python: user_var_value = self._codec.encode(user_var_value)
	encodedUserValue := m.aesCodec.Encode(userVarValue)
	// Python: session_var_value = self._codec.encode(session_var_value)
...
//
// Python: VariableManager._check_user_and_scope_id
func (m *VariableManager) checkUserAndScopeID(userID string, scopeID string, context string) {
	if userID == "" || strings.TrimSpace(userID) == "" {
		// Python: memory_logger.error("Check user and scope id operation failed, user ID is empty", ...)
		logger.Error(logComponent).
	...
	if scopeID == "" || strings.TrimSpace(scopeID) == "" {
		// Python: memory_logger.error("Check user and scope id operation failed, scope ID is empty", ...)
		logger.Error(logComponent).
			Str("event_type", "MEMORY_RETRIEVE").
			Str("memory_type", "variable").
			Str("user_id", userID).
...
//
// Python: VariableManager._check_exist
