...
- [ ] **Step 1: 在 card.go 结构体区块添加 CardInterface 接口定义**
...
- [ ] **Step 2: 在 card.go 全局变量区块添加编译时验证**
...
- [ ] **Step 4: 在 card_test.go 添加 CardInterface 满足测试**
...
- [ ] **Step 1: TeamCardInterface 嵌入 schema.CardInterface，删除 GetBaseCard 声明**
...
- [ ] **Step 2: 删除 TeamCard.GetBaseCard() 和 EventDrivenTeamCard.GetBaseCard() 方法**
...
- [ ] **Step 3: 更新 TeamCard 编译时验证（CardInterface）**
...
- [ ] **Step 1: idToCard 类型从 `*schema.BaseCard` 改为 `schema.CardInterface`**
...
- [ ] **Step 3: dispatchAdd 删除 resourceCard 参数**
...
- [ ] **Step 5: getCardType 从 reflect.TypeOf 改为 type switch**
...
- [ ] **Step 6: innerValidateResourceCard 签名变更**
...
- [ ] **Step 8: GetSysOpToolCards 返回值变更**
...
- [ ] **Step 9: session 类型从 any 改为 decorator.TracerSession**
...
- [ ] **Step 10: 合并 innerGetResourcesByProvider → innerGetResources**
...
- [ ] **Step 11: AddMcpServer 中 idToCard.Set 改为存 McpToolCard 本身**
...
- [ ] **Step 12: GetToolInfos 修复 — 使用 ToolCard 本身调用 getCardType**
...
- [ ] **Step 2: RemoveWorkflow 改造**

同 RemoveAgent 模式，结果转为 `[]*schema.WorkflowCard`。

- [ ] **Step 3: RemoveTool 改造**
...
- [ ] **Step 4: RemoveModel / RemovePrompt / RemoveSysOperation 改造**

统一走 innerRemoveResources，各自转换返回类型。

- [ ] **Step 5: RemoveAgentTeam 改造**
...
- [ ] **Step 2: GetWorkflow / GetTool / GetModel / GetPrompt / GetAgentTeam 改造**
...
- [ ] **Step 1: 更新 resource_manager_test.go**
...
- [ ] **Step 1: 更新 common/schema/doc.go**

在包功能概述中提及 CardInterface 接口，在文件目录中确认 card.go 的职责描述。

- [ ] **Step 2: 更新 multi_agent/schema/doc.go**
