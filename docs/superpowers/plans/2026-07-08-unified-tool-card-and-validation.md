...
**Architecture:** (1) Param 补齐 3 字段 → (2) ParseJSONSchemaMap 转换器 → (3) BuildToolCard 统一建卡 → (4) 5 个工具改用 InvokeFunction 包装 + BuildToolCard 建卡 → (5) 提示词对齐 Python

**Tech Stack:** Go 1.23+, schema.Param, InvokeFunction[I,O], SchemaUtils.FormatWithSchema
...
- [ ] **Step 1: 在 Param struct 中新增 3 个字段**
...
- [ ] **Step 2: 修改 paramToSchema 输出 AdditionalProperties/MinItems/MaxItems**
...
- [ ] **Step 3: 修改 Param.Validate() 校验新字段**
...
- [ ] **Step 1: 找到 bestEffert 和 failureDestination 属性，补上 type 字段**
...
- [ ] **Step 2: 运行 prompts/tools 测试确认不破坏**
...
- [ ] **Step 1: 写 ParseJSONSchemaMap 的失败测试**
...
- [ ] **Step 3: 实现 ParseJSONSchemaMap + parsePropertyToParam**
...
### Task 4: 实现 BuildToolCard 统一建卡函数
...
- [ ] **Step 1: 在 foundation/tool/base.go 新增 NewToolCardWithID**
...
### Task 5: SearchToolsTool 改用 InvokeFunction 包装 + BuildToolCard
...
- [ ] **Step 2: 更新 search_tools_test.go 适配新接口**
...
- [ ] **Step 4: 检查 progressive_test.go 是否引用了 SearchToolsTool struct**
...
### Task 6: LoadToolsTool 改用 InvokeFunction 包装 + BuildToolCard
...
### Task 7: SessionsListTool/SessionsSpawnTool/SessionsCancelTool 改用 InvokeFunction + BuildToolCard
...
- [ ] **Step 2: 更新 session_tools_test.go 适配新接口**

- [ ] **Step 3: 检查是否有其他文件引用 SessionsXxxTool struct**
...
- [ ] **Step 3: 检查 ProgressiveToolRail 是否受影响**
