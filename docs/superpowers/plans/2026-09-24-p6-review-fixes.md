...
- [ ] **Step 5: 更新 core/config/doc.go（父目录）和 core/doc.go**
...
- [ ] **Step 1: 在 io_schema.go 结构体区块添加 MemoryItem 接口**
...
- [ ] **Step 2: 为 ReasoningBankRetrievedMemory 添加 FormatMemoryString 方法**
...
- [ ] **Step 3: 为 ReMeRetrievedMemory 添加 FormatMemoryString 方法**
...
- [ ] **Step 4: 为 ACEMemory 添加 FormatMemoryString 方法**
...
- [ ] **Step 1: RB recall op 存入改为 []MemoryItem**
...
- [ ] **Step 2: ACE recall op 存入改为 []MemoryItem**

`ace/run.go` 同理。

- [ ] **Step 3: ReMe recall op 存入改为 []MemoryItem**

`reme/run.go` 的 RecallMemoryOp.Execute 同理。

- [ ] **Step 4: ReMe RerankOp/RewriteOp 适配 []MemoryItem**
...
- [ ] **Step 1: 在 task_memory_service.go 结构体区块添加返回值 struct**
...
- [ ] **Step 1: 字段改回具体类型（D-01），取消 WithServices 构造函数**
...
- [ ] **Step 2: TaskMemoryServiceConfig 删除运行时参数（D-04/D-05 关联）**
...
- [ ] **Step 3: createRetrieveFlow 从 config 实时读取（D-04/D-05）**
...
- [ ] **Step 4: createSummaryFlow 从 config 实时读取（D-04/D-05）**
...
- [ ] **Step 11: 添加 GetPlaybook / ClearPlaybook（D-10）**
...
- [ ] **Step 1: RunTrialsInput 加 persist + memoryService 字段（D-11）**
...
- [ ] **Step 4: 添加 diversity prompt + COMBINED_MATTS_PROMPT（D-14）**
...
- [ ] **Step 3: 添加 AutoConfigure 方法（D-08）**
...
- [ ] **Step 1: 删除 llm_wrapper.go 中的编译期接口断言（D-03）**
...
- [ ] **Step 1: 更新 core/doc.go 添加 config/ 子包**

- [ ] **Step 2: 更新 service/doc.go 反映签名变更**
