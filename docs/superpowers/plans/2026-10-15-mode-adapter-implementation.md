# 10.3.4-6 模式适配器（DeepAdapter + CodeAdapter）实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 实现 DeepAdapter 和 CodeAdapter 骨架，更新工厂路由，使 CreateAdapter 返回真实适配器实例而非 "尚未实现" 错误。

**Architecture:** DeepAdapter 直接实现 AgentAdapter 接口，8 个方法中可实现的逻辑（参数校验、session 管理、intent 分发、前缀匹配）直接写，依赖 agentcore/辅助模块的逻辑用 `⤵️ 10.3.7-11` 回填注释标记。CodeAdapter 内嵌 `*DeepAdapter` 组合委托，仅覆盖 CreateInstance。工厂路由从返回错误改为返回真实实例。

**Tech Stack:** Go 1.23+, github.com/uapclaw/uapclaw-go/internal/swarm/schema, github.com/uapclaw/uapclaw-go/internal/common/logger

**Spec:** `docs/superpowers/specs/2026-10-15-mode-adapter-design.md`
...
| 操作 | 文件路径 | 职责 |
|------|---------|------|
| 新建 | `internal/swarm/server/adapter/deep_adapter.go` | DeepAdapter 结构体 + 8 个接口方法 + 4 个 session 辅助方法 + 2 个 params 解析辅助 |
| 新建 | `internal/swarm/server/adapter/deep_adapter_test.go` | DeepAdapter 单元测试 |
| 新建 | `internal/swarm/server/adapter/code_adapter.go` | CodeAdapter 结构体 + CreateInstance 覆盖 + 7 个委托方法 |
| 新建 | `internal/swarm/server/adapter/code_adapter_test.go` | CodeAdapter 单元测试 |
| 修改 | `internal/swarm/server/adapter/factory.go` | 更新 CreateAdapter 路由 |
| 修改 | `internal/swarm/server/adapter/factory_test.go` | 更新测试用例 |
| 修改 | `internal/swarm/server/adapter/doc.go` | 更新文件目录 |
| 修改 | `IMPLEMENTATION_PLAN.md` | 10.3.4-6 状态更新 |

---

### Task 1: DeepAdapter 结构体与构造函数

**Files:**
- Create: `internal/swarm/server/adapter/deep_adapter.go`

- [ ] **Step 1: 编写 DeepAdapter 结构体和 NewDeepAdapter 构造函数**

在 `internal/swarm/server/adapter/deep_adapter.go` 中创建文件，包含：
- package 声明 + import（context, encoding/json, fmt, strings, schema, logger）
- DeepAdapter 结构体（所有字段，按规范排列：当前可用字段 → ⤵️ 模型与配置 → ⤵️ Rails → ⤵️ 运行时，每个 interface{} 字段带 `⤵️ 10.3.7-11:` 回填注释）
- `NewDeepAdapter()` 构造函数
- 分隔注释按 Go 编码规范（`// ──────────────────────────── 结构体 ────────────────────────────` 等）

对应设计文档 Section 4.1-4.2，对应 Python `JiuWenClawDeepAdapter.__init__` (line 472-551)。
...
Expected: 编译成功（可能有 unused import 警告，但无错误）

- [ ] **Step 3: 提交**

```bash
git add internal/swarm/server/adapter/deep_adapter.go
git commit -m "feat(adapter): 新增 DeepAdapter 结构体与构造函数

全量字段 interface{} 占位，⤵️ 10.3.7-11 回填注释标注
对应 Python JiuWenClawDeepAdapter.__init__ (line 472-551)"
```

---

### Task 2: DeepAdapter 非导出方法（session 活跃计数 + params 解析辅助）

**Files:**
- Modify: `internal/swarm/server/adapter/deep_adapter.go`

- [ ] **Step 1: 在 deep_adapter.go 的非导出函数区追加 6 个辅助函数**

对应设计文档 Section 4.4-4.5。
...
```bash
git add internal/swarm/server/adapter/deep_adapter.go
git commit -m "feat(adapter): DeepAdapter 新增 session 活跃计数和 params 解析辅助

- mark/unmark/is/otherActiveSessions: Counter 语义的 session 管理
- parseParams/paramsString: json.RawMessage → map[string]any 解析
对应 Python _mark_session_active 等方法 (line 576-610)"
```

---

### Task 3: DeepAdapter 接口方法实现（8 个方法骨架）

**Files:**
- Modify: `internal/swarm/server/adapter/deep_adapter.go`

- [ ] **Step 1: 在导出函数区追加 8 个接口方法**

按设计文档 Section 4.3.1-4.3.8 完整实现。每个方法严格对齐 Python 源码步骤编号，可实现的逻辑直接写，依赖 agentcore 的用 `⤵️ 10.3.7-11` 标注。

追加在 `NewDeepAdapter()` 之后、非导出函数区之前：
...
```bash
git add internal/swarm/server/adapter/deep_adapter.go
git commit -m "feat(adapter): DeepAdapter 实现 8 个接口方法骨架

每个方法严格对齐 Python 源码步骤编号：
- CreateInstance: 步骤 1-25（实现 2/3/9/10, 其余 ⤵️ 回填）
- ReloadAgentConfig: 步骤 1-13（全部 ⤵️ 回填）
- ProcessMessageImpl: 步骤 1-21（实现 1/4-6, 其余 ⤵️ 回填）
- ProcessMessageStreamImpl: 步骤 1-21（实现 1/4-6, 其余 ⤵️ 回填）
- ProcessInterrupt: 步骤 1-11（实现 1-6/8-9 cancel, 其余 ⤵️ 回填）
- HandleUserAnswer: 步骤 1-8（实现 1-5/7-8, 其余 ⤵️ 回填）
- HandleHeartbeat: 步骤 1-5（实现 1-2/4-5, 其余 ⤵️ 回填）
- Cleanup: 步骤 1（⤵️ 回填）"
```

---

### Task 4: DeepAdapter 单元测试

**Files:**
- Create: `internal/swarm/server/adapter/deep_adapter_test.go`

- [ ] **Step 1: 编写 DeepAdapter 测试文件**

覆盖设计文档 Section 6.1 所有测试用例。
...
Run: `cd /home/opensource/uapclaw-gateway && go test -v -tags test ./internal/swarm/server/adapter/ -run "TestNewDeepAdapter|TestDeepAdapter_|TestParseParams|TestParamsString"`
Expected: 全部 PASS

- [ ] **Step 3: 提交**

```bash
git add internal/swarm/server/adapter/deep_adapter_test.go
git commit -m "test(adapter): DeepAdapter 单元测试

覆盖：构造函数默认值、接口满足性、session活跃计数、
ProcessInterrupt intent分支、HandleUserAnswer前缀分发、
HandleHeartbeat前缀检查、未初始化检查、parseParams/paramsString"
```

---

### Task 5: CodeAdapter 实现

**Files:**
- Create: `internal/swarm/server/adapter/code_adapter.go`

- [ ] **Step 1: 编写 CodeAdapter 结构体和全部接口方法**
...
git commit -m "feat(adapter): 新增 CodeAdapter 组合委托 DeepAdapter

- 结构体：deep + 编码专有 rails(interface{}占位)
- 仅覆盖 CreateInstance（dreaming_mode=code, isCodeAgent=true）
- 其余 7 方法委托 DeepAdapter
对应 Python JiuwenClawCodeAdapter (line 154-342)"
```

---

### Task 6: CodeAdapter 单元测试

**Files:**
- Create: `internal/swarm/server/adapter/code_adapter_test.go`

- [ ] **Step 1: 编写 CodeAdapter 测试文件**
...
Run: `cd /home/opensource/uapclaw-gateway && go test -v -tags test ./internal/swarm/server/adapter/ -run "TestNewCodeAdapter|TestCodeAdapter_"`
Expected: 全部 PASS

- [ ] **Step 3: 提交**

```bash
git add internal/swarm/server/adapter/code_adapter_test.go
git commit -m "test(adapter): CodeAdapter 单元测试

覆盖：构造函数默认值（isCodeAgent=true/forceEnglish=true）、接口满足性编译检查"
```
...
**Files:**
- Modify: `internal/swarm/server/adapter/factory.go`
- Modify: `internal/swarm/server/adapter/factory_test.go`

- [ ] **Step 1: 更新 factory.go 的 CreateAdapter 函数**

将原来的 "尚未实现" 错误替换为真实适配器创建：
...
- [ ] **Step 2: 更新 factory_test.go 测试用例**

将原来验证 "尚未实现" 错误的测试改为验证返回正确类型：

```go
// TestCreateAdapter 测试工厂函数路由。
func TestCreateAdapter(t *testing.T) {
	tests := []struct {
		...
		{"harness+agent→DeepAdapter", "harness", "agent", "*adapter.DeepAdapter", false},
		{"harness+空→DeepAdapter", "harness", "", "*adapter.DeepAdapter", false},
		{"harness+code→CodeAdapter", "harness", "code", "*adapter.CodeAdapter", false},
		{"空sdk+agent→DeepAdapter", "", "agent", "*adapter.DeepAdapter", false},
		{"pi→error", "pi", "agent", "", true},
		{"未知sdk→error", "unknown", "agent", "", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			adapter, err := CreateAdapter(tt.sdk, tt.mode)
			...
			if typeName != tt.wantType {
				t.Errorf("CreateAdapter() type = %s, want %s", typeName, tt.wantType)
			}
		})
	}
}
...
Run: `cd /home/opensource/uapclaw-gateway && go test -v -tags test ./internal/swarm/server/adapter/ -run TestCreateAdapter`
Expected: 全部 PASS
...
git commit -m "feat(adapter): CreateAdapter 工厂路由更新

- mode=code → NewCodeAdapter()
- 其余 mode → NewDeepAdapter()
- 去掉 '尚未实现' 错误，返回真实适配器实例"
```
...
```bash
git add internal/swarm/server/adapter/doc.go
git commit -m "docs(adapter): 更新 doc.go 文件目录，添加 DeepAdapter/CodeAdapter 条目"
```

---

### Task 9: IMPLEMENTATION_PLAN 状态更新

**Files:**
- Modify: `IMPLEMENTATION_PLAN.md`

- [ ] **Step 1: 更新 10.3.4-6 状态为 ✅**

将 `| 10.3.4-6 | ☐ | 模式适配器` 改为 `| 10.3.4-6 | ✅ | 模式适配器`

- [ ] **Step 2: 提交**

```bash
git add IMPLEMENTATION_PLAN.md
git commit -m "docs: IMPLEMENTATION_PLAN 10.3.4-6 模式适配器状态更新为 ✅"
```
...
Run: `cd /home/opensource/uapclaw-gateway && go test -v -tags test ./internal/swarm/server/adapter/`
Expected: 全部 PASS

- [ ] **Step 2: 运行 adapter 包覆盖率检查**

Run: `cd /home/opensource/uapclaw-gateway && go test -tags test -cover ./internal/swarm/server/adapter/`
Expected: 覆盖率 ≥ 85%（DeepAdapter/CodeAdapter 骨架方法大部分有分支测试）

- [ ] **Step 3: 运行项目级测试确认无回归**

Run: `cd /home/opensource/uapclaw-gateway && go test -tags test ./... 2>&1 | tail -20`
Expected: 无 FAIL（可能有其他包的既有问题，但 adapter 包自身必须 PASS）
