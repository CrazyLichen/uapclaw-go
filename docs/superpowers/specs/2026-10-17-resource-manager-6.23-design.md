...
| 19 | AddToolServer 签名 | 直接参数 `(ctx, serverConfig *mcp.McpServerConfig, expiryTime *float64)`，与 Python 一致 |
...
方法（非导出，与 Python `_register_resource_provider`/`_get_resource`/`_unregister_resource_provider` 对齐）：
...
| `add_tool_server(server_config, expiry_time)` | `AddToolServer(ctx context.Context, serverConfig *mcp.McpServerConfig, expiryTime *float64) ([]*mcp.McpToolCard, error)` |
...
`AddToolServer` 实现要点（签名：`AddToolServer(ctx context.Context, serverConfig *mcp.McpServerConfig, expiryTime *float64) ([]*mcp.McpToolCard, error)`，直接参数与 Python `add_tool_server(server_config, expiry_time)` 一致）：
...
- `RemoveModel(modelID string, opts ...ResourceOption) ([]string, error)` — 返回 ID 列表（Python 实际行为：model 不在 idReturnTypes 中但因 add_model 无 resource_card，`_id_to_card` 无缓存，返回 Ok(None)；Go 返回 ID 列表更实用）
...
**MCP Server 操作**（AddToolServer 直接参数，其余 Functional Options 模式）：
- `AddMcpServer(ctx context.Context, serverConfig *McpServerConfig, expiryTime *float64) ([]*McpToolCard, error)` — 直接参数，与 Python `add_tool_server(server_config, expiry_time)` 一致
- `RefreshMcpServer(ctx context.Context, serverID string, opts ...McpOption) ([]*McpToolCard, error)`
- `RemoveMcpServer(ctx context.Context, serverID string, opts ...McpOption) ([]string, error)` — 返回移除的 tool ID 列表，与 Python `Result[str, Exception]` 对齐
...
McpOption 包含：`WithMcpServerName`/`WithMcpTag`/`WithMcpSkipIfNotExists`/`WithMcpForce`/`WithMcpSession`（`WithMcpExpiryTime` 已移除，expiryTime 作为 AddToolServer 直接参数）
